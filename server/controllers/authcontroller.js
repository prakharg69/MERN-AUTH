...
export const registered = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      to: email,
      subject: "Welcome To prakharworld ",
      text: `hey baby girl ${name} i miss you so much this mail is send you by code lots of love by owner PrakharLord `,
    };
...
export const sendVerifyOtp = async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: "Verification email just send to email",
    });
  } catch (error) {
    return res.json({ success: false, message: error.message });
  }
...
export const sendRestOtp = async (req, res) => {
  ...
  try {
    ...
    const mailOptions = {
      ...
      text: `Your Otp is ${otp} verify your account using this otp and rest the password `,
    };
    await transporter.sendMail(mailOptions);
    return res.json({ success: true, message: "otp send to user" });
  } catch (error) {
    return res.status(500).json({ success: false, message: error.message });
  }
};
